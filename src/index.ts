...
import { SessionLogTailer, type Diagnostic, type RawRecord } from './reader.js'
import { buildGraph, type ExecutionGraph } from './graph.js'
import {
  buildVersionChains,
  summarizeEffects,
  type Effect,
  type EffectKind,
  type FileVersionChain,
} from './effects.js'
import { buildManifest, type LedgerManifest } from './merkle.js'
...
export interface LedgerSnapshot {
  ...
  integrity: { frameRoot: string; recordRoot: string; frames: number; records: number; ms: number }
}
 ...
 */
export async function readLedger(logPath: string): Promise<LedgerSnapshot> {
  const started = Date.now()
  const stats = { frames: 0, records: 0, bytes: 0, pendingBytes: 0, diagnostics: [] as Diagnostic[] }
  ...
  const raw: RawRecord[] = (values ?? []).map((v) => ({
    value: v.value,
...
export async function attest(logPath: string): Promise<LedgerManifest> {
...
export async function findSessionLogs(home = process.env.DSH_HOME ?? join(homedir(), '.dsh')): Promise<SessionLogInfo[]> {
  const root = join(home, 'sessions')
  const out: SessionLogInfo[] = []
  let projects: string[]
  try {
 ...
 * a record in the log.
 */
export function renderSummary(snapshot: LedgerSnapshot): string {
  ...
  lines.push(
    `sandbox   ${gov.sandboxMode ?? '?'}  approval ${gov.approvalPolicy ?? '?'}  permission ${gov.permissionPreset ?? '?'}`,
  )
  lines.push(
    `log       ${ingest.frames} frames, ${ingest.records} records, ${ingest.bytes} bytes in ${ingest.ms}ms` +
  ...
  for (const [kind, count] of Object.entries(effectSummary).sort((a, b) => b[1] - a[1])) {
    if (count > 0) lines.push(`  ${kind.padEnd(11)} ${count}`)
  }
  ...
  for (const chain of sorted.slice(0, 15)) {
    ...
    lines.push(
      `  ${chain.created ? 'new  ' : chain.preexisting ? 'pre  ' : 'mod  '} ${chain.steps.length}x ${chain.path}${flag}`,
    )
  }
  ...
  if (graph.anomalies.length > 0) {
    ...
    if (graph.anomalies.length > 15) lines.push(`  … and ${graph.anomalies.length - 15} more`)
  }
  lines.push('')
