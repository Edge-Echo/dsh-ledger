...
export type FrameWalk =
  | { kind: 'frame'; span: FrameSpan }
  /** The bytes present are a valid prefix of a frame — the writer may still be appending. */
  | { kind: 'incomplete'; start: number; needBytes: number }
  /** The bytes at `start` cannot begin a frame. */
  | { kind: 'corrupt'; start: number; reason: string }
  /** No bytes available at `start`. */
  | { kind: 'eof' }

function readU24LE(buf: Buffer, at: number): number {
 ...
 */
export function walkFrameAt(buf: Buffer, offset: number): FrameWalk {
  if (offset >= buf.length) return { kind: 'eof' }
  ...
  if (buf.length - offset >= 8) {
    ...
    if (magic >= SKIPPABLE_MAGIC_MIN && magic <= SKIPPABLE_MAGIC_MAX) {
      ...
      if (end > buf.length) return { kind: 'incomplete', start: offset, needBytes: end - buf.length }
  ...
  if (buf.length - offset < 5) return { kind: 'incomplete', start: offset, needBytes: 5 - (buf.length - offset) }
  ...
  if (headerEnd > buf.length) return { kind: 'incomplete', start: offset, needBytes: headerEnd - buf.length }
  ...
  if (fcsWidth === 1) {
    ...
    if (hi !== 0) contentSize = undefined // beyond Number.MAX_SAFE_INTEGER for our purposes
    else contentSize = lo
  }
  ...
  let blocks = 0
  for (;;) {
    if (cursor + 3 > buf.length) return { kind: 'incomplete', start: offset, needBytes: cursor + 3 - buf.length }
    ...
    blocks++
    if (lastBlock) break
    if (blocks > 1_000_000) return { kind: 'corrupt', start: offset, reason: 'implausible block count' }
  }

  if (hasChecksum) {
    if (cursor + 4 > buf.length) return { kind: 'incomplete', start: offset, needBytes: cursor + 4 - buf.length }
    cursor += 4
  }
...
export function decodeFrame(buf: Buffer, span: FrameSpan): string {
  if (span.skippable) return ''
 ...
 */
export function findNextFrameMagic(buf: Buffer, from: number): number {
