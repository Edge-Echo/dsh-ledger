 ...
 */
export function canonicalRecordFields(p) {
    return [
        p.sessionId,
        p.ts,
        p.kind,
...
function variablePaths(command) {
    ...
    for (const [name, value] of raw) {
        const expanded = value.replace(/\$([A-Za-z_]\w*)/g, (whole, ref) => raw.get(ref) ?? whole);
...
function statementAround(command, index) {
    const boundary = (c) => c === ';' || c === '\n' || c === '|' || c === '}' || c === '{';
    let start = index;
    while (start > 0 && !boundary(command[start - 1]))
        start--;
    let end = index;
    while (end < command.length && !boundary(command[end]))
        end++;
 ...
 */
function classifyRecursiveDelete(command, cwd, matchIndex) {
 ...
 */
export function assessEffect(effect, cwd) {
    const flags = [];
    let severity = 'info';
    const raise = (s) => {
    ...
    if (sensitiveHit)
        flags.push(`sensitive-path:${sensitiveHit[1]}`);
    switch (effect.kind) {
        case 'read': {
        ...
        case 'shell': {
            ...
            if (effect.undecidable)
                flags.push('effects-undecidable');
            raise('medium');
 ...
 */
function callStatements(call, cwd) {
    let severity = 'info';
    const flags = new Set();
    ...
    for (const effect of call.effects) {
        ...
        for (const flag of f)
            flags.add(flag);
        if (effect.kind === 'read' && effect.path)
            filesRead.add(effect.path);
        if ((effect.kind === 'write' || effect.kind === 'edit') && effect.path)
            filesWritten.add(effect.path);
        if (effect.kind === 'network' && effect.source)
            network.add(effect.source);
    }
...
const effectSummaryLine = (effects) => effects
    .map((e) => {
    if (e.kind === 'shell')
        return `shell: ${(e.source ?? '').slice(0, 120)}`;
    if (e.path)
        return `${e.kind} ${e.path}`;
    if (e.source)
        return `${e.kind} ${e.source.slice(0, 80)}`;
 ...
 */
export function toCompliancePayloads(graph) {
    ...
    const push = (ts, rank, payload) => {
        stamped.push({
            ts,
            rank,
            payload: { sessionId, ts, sourceType: 'dsh-ledger', ...payload },
        });
    };
    ...
    for (const anomaly of graph.anomalies) {
        if (anomaly.kind !== 'permission-change' &&
 ...
 */
export function documentHash(prevHash, line) {
 ...
 */
export function complianceJsonl(payloads, opts = {}) {
    const hashChain = opts.hashChain !== false;
    const lines = [];
    lines.push(JSON.stringify({ schema: COMPLIANCE_SCHEMA, generatedAt: Date.now(), count: payloads.length, hashChain }));
    let prevHash = '';
...
export function verifyComplianceChain(text) {
    ...
    let header;
    try {
        header = JSON.parse(raw[0]);
    }
    catch (err) {
        return { valid: false, lines: raw.length, records: 0, issues: [`header is not JSON: ${err.message}`] };
    }
    ...
    for (let i = 1; i < raw.length; i++) {
        let line;
        try {
            line = JSON.parse(raw[i]);
        }
        catch (err) {
            issues.push(`line ${i + 1}: not JSON (${err.message})`);
            continue;
        }
        records++;
 ...
 */
export async function verifyComplianceWithUpstream(text) {
    try {
        const mod = (await import('dsh-audit-trail'));
...
export function severityHistogram(payloads) {
...
export function effectKindTotals(effects) {
    const out = {};
