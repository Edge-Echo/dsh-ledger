...
import { attest, findSessionLogs, readLedger, renderSummary, signManifest, verifyManifest, verifyManifestSignature, } from './index.js';
import { complianceJsonl, effectKindTotals, SEVERITIES, severityHistogram, severityRank, toCompliancePayloads, verifyComplianceWithUpstream, } from './compliance.js';
...
function parseArgs(argv) {
    ...
    const positional = [];
    const flags = new Map();
    const isFlag = (token) => token.length > 1 && token.startsWith('-');
...
async function resolveLog(input) {
    try {
    ...
    if (matches.length === 0)
        die(`no session log matches "${input}"`);
    if (matches.length > 1) {
...
async function cmdReport(logArg, json) {
...
async function cmdClassify(logArg, json, minSeverity) {
    const path = await resolveLog(logArg);
    const snapshot = await loadSnapshot(path);
    ...
    for (const record of flagged.slice(0, 30)) {
        ...
        if (record.flags.length > 0)
            process.stdout.write(`             flags: ${record.flags.join(', ')}\n`);
    }
    if (flagged.length > 30)
        process.stdout.write(`  … and ${flagged.length - 30} more\n`);
}
function sidecarLines(snapshot) {
    const lines = [];
...
function verifyGuide(packName, manifest) {
...
async function cmdPack(logArg, out, keyPath, title) {
    const path = await resolveLog(logArg);
    const snapshot = await loadSnapshot(path);
    const payloads = toCompliancePayloads(snapshot.graph);
    const compliance = complianceJsonl(payloads);
    let manifest = await attest(path);
    let signed = false;
    if (keyPath) {
        manifest = signManifest(manifest, readFileSync(resolve(keyPath), 'utf8'));
        signed = verifyManifestSignature(manifest);
        if (!signed)
            die('the signature did not verify after signing; refusing to write a pack that claims to be signed');
    }
    const histogram = severityHistogram(payloads);
    const report = [
        `# ${title}`,
    ...
    if (target.toLowerCase().endsWith('.zip')) {
        ...
        for (const entry of entries)
            writeFileSync(join(target, entry.name), entry.data);
    }
...
async function cmdVerify(target, manifestPath, logPath) {
    ...
    try {
        ...
        die(`could not read ${resolved}: ${err.message}`);
    }
    ...
    if (chain.maxSeverity)
        process.stdout.write(` maxSeverity=${chain.maxSeverity}`);
    process.stdout.write('\n');
    for (const issue of chain.issues.slice(0, 10))
        process.stdout.write(`  issue: ${issue}\n`);
    let ok = chain.valid;
    const manifestFile = manifestPath ?? (statSync(resolved).isDirectory() ? join(resolved, 'manifest.json') : undefined);
...
async function main() {
    ...
    const minSeverity = (flag(args, 'min-severity') ?? 'info');
    if (!SEVERITIES.includes(minSeverity))
        die(`--min-severity must be one of ${SEVERITIES.join('|')}`);
    switch (args.command) {
        case 'list':
            return cmdList(json);
        case 'report': {
