...
export class MerkleTree {
    ...
    prove(index) {
        ...
        const subtreeRoot = (lo, hi) => {
            const n = hi - lo;
            if (n === 1)
                return this.levels[0][lo];
            const depth = Math.log2(n);
            if (Number.isInteger(depth) && lo % n === 0)
                return this.levels[depth][lo / n];
    ...
    static verify(proof, leafHash, root) {
        let acc = leafHash;
    ...
    static verifyLeaf(proof, leaf, root) {
 ...
 */
export function extractRecordMeta(line, prefixLimit = 4096) {
 ...
 * content.
 */
export async function buildManifest(logPath, opts = {}) {
        ...
        try {
            ...
            for (let i = 0; i <= text.length; i++) {
                if (i === text.length || text.charCodeAt(i) === 0x0a) {
                    ...
                    if (meta.type === 'session' && meta.sessionId)
 ...
 */
export async function verifyManifest(logPath, expected) {
    ...
    let mismatch;
    if (manifest.bytes !== expected.bytes)
        mismatch = 'bytes';
    else if (manifest.frames !== expected.frames)
        mismatch = 'frames';
    else if (manifest.records !== expected.records)
        mismatch = 'records';
    else if (manifest.frameRoot !== expected.frameRoot)
        mismatch = 'frameRoot';
    else if (manifest.recordRoot !== expected.recordRoot)
        mismatch = 'recordRoot';
    void frameTree;
    void recordTree;
...
export function verifyManifestSignature(manifest) {
    ...
    if (!pem)
        return false;
    try {
        return verify(null, Buffer.from(signableManifest(manifest), 'utf8'), createPublicKey(pem), Buffer.from(manifest.signature, 'base64'));
    }
    catch {
        return false;
    }
